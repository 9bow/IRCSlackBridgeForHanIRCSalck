...
var Bot = function () {
  function Bot(options) {
    ...
    var defaultUrl = 'https://picsum.photos/seed/$username/64/64';
  ...
  _createClass(Bot, [{
    ...
    value: function parseText(text) {
      ...
      return text.replace(/\n|\r\n|\r/g, ' ').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/<!channel>/g, '@channel').replace(/<!group>/g, '@group').replace(/<!everyone>/g, '@everyone').replace(/<#(C\w+)\|?(\w+)?>/g, function (match, channelId, readable) {
